...
# circle dataset
def generate_circle_points_noisy(radius, num_points):
...
def generate_circle_points(radius, num_points):
	...
	for i in range(num_points):
		...
		r = 0
		while r < radius:
			r = (radius+3) * math.sqrt(random.random())
		classB[i][0] = math.cos(p) * r
		classB[i][1] = math.sin(p) * r
...
classA, classB = generate_circle_points_noisy(1, 40)
testSVM = svm.SVM(classA, classB, C=1, kernel='RBF', degree=3, sigma=.7, filename='noisy_circle_largeRBF')
